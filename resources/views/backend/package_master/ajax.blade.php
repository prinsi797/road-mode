        ...
        <tbody>
            @foreach ($data as $v)
                <tr class="row_{{ $v->id }}">
                    ...
                    <td>{{ ++$record_id }}</td>
                    <td><a href="{{ $v->show_route }}">{{ $v->pack_name }}</a></td>
                    <td>{{ $v->pack_code }}</td>
                    <td>{{ $v->service->sc_name ?? 'N/A' }}</td>
                    {{-- <td>{{ $v->category->sc_name }}</td> --}}
                    <td>{{ $v->pack_net_amt }}</td>
                    <td>{{ $v->pack_duration }}</td>
                    <td>
                        @if ($v->status == 'active')
                    ...
                    <td>
                        @if ($v->deleted_at == null)
                            <a href="{{ route('admin.package_master.edit', ['encrypted_id' => Crypt::encryptString($v->id)]) }}"
